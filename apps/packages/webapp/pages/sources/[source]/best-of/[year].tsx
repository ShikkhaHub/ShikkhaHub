...
import type {
  ...
  ArchiveData,
  ArchiveIndexData,
} from '@shikkhahub/shared/src/graphql/archive';
import {
  ARCHIVE_INDEX_QUERY,
  ARCHIVE_QUERY,
  ArchivePeriodType,
...
import { ArchiveFeedPage } from '@shikkhahub/shared/src/components/archive/ArchiveFeedPage';
...
} from '@shikkhahub/shared/src/lib/archive';
import type { SourceData } from '@shikkhahub/shared/src/graphql/sources';
import {
  SOURCE_QUERY,
  SourceType,
...
function getJsonLd({
  ...
  year,
  archive,
}: Pick<PageProps, 'sourceId' | 'sourceName' | 'year' | 'archive'>): string {
...
const SourceYearlyArchivePage = ({
  ...
  const encodedSource = encodeURIComponent(sourceId);
  const jsonLd = getJsonLd({ sourceId, sourceName, year, archive });
